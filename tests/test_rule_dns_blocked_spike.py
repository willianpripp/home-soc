...
def _blocked_burst(conn, client, ts, count):
...
def test_fires_for_a_client_spiking_past_3x_and_the_floor(conn):
    ...
    # which is both >= 20 absolute and >= 3x the 5/day baseline.
...
def test_does_not_fire_for_high_but_proportionate_volume(conn):
    ...
    # prior week's own daily average, so this is volume, not a spike.
